...
export default function Home() {
  ...
  useEffect(() => {
    setRevealed(true);
    setSearchTerm(TOPICS[topicIndex].q);
  ...
  // Search input change
  useEffect(() => {
    if (searchTerm !== TOPICS[topicIndex].q) {
      // do nothing if user typing
    } else {
      setSearchTerm(TOPICS[topicIndex].q);
    }
  }, [topicIndex]);
          ...
          <div className={`swap ${swapState}`}>
            <div id="cards" className="gap-5 grid grid-cols-1 md:grid-cols-3">
              {topic.posts.map((p, i) => (
                ...
                >
                  <div
                    className="flex flex-shrink-0 justify-center items-center rounded-lg w-[64px] h-[64px] font-bold text-white text-sm"
